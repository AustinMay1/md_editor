...
class HtmlHandler {
    private markdown_change: Markdown = new Markdown();
    ...
    public TextChangeHandler(id: string, output: string): void {
        ...
        if (markdown !== null) {
            markdown.onkeyup = (e) => {
                this.RenderHTMLContent(markdown, markdown_output);
            };
            window.onload = (e) => {
                this.RenderHTMLContent(markdown, markdown_output);
            };
    ...
    public RenderHTMLContent(
        ...
        markdown_output: HTMLLabelElement
    ) {
        if (markdown.value) {
            markdown_output.innerHTML = this.markdown_change.ToHTML(
                markdown.value
            );
        } else {
...
class TagTypeHTML {
    ...
    constructor() {
        ...
        this.tag_type.set(TagType.H4, "h4");
        this.tag_type.set(TagType.H5, "h5");
        this.tag_type.set(TagType.H6, "h6");
...
class H4Visitor extends VisitorBase {
    constructor() {
        super(TagType.H4, new TagTypeHTML());
    }
}

class H5Visitor extends VisitorBase {
    constructor() {
        super(TagType.H5, new TagTypeHTML());
    }
}

class H6Visitor extends VisitorBase {
    constructor() {
        super(TagType.H6, new TagTypeHTML());
    }
}
...
class H4Handler extends ParseChainHandler {
    constructor(document: IMarkdownDocument) {
        super(document, "#### ", new H4Visitor());
    }
}

class H5Handler extends ParseChainHandler {
    constructor(document: IMarkdownDocument) {
        super(document, "##### ", new H5Visitor());
    }
}

class H6Handler extends ParseChainHandler {
    constructor(document: IMarkdownDocument) {
...
class ChainFactory {
    Build(document: IMarkdownDocument): ParseChainHandler {
        ...
        let H4: H4Handler = new H4Handler(document);
        let H5: H5Handler = new H5Handler(document);
        let H6: H6Handler = new H6Handler(document);
